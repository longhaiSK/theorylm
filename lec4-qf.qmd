...
::: {#def-quadratic-form-matrix name="Quadratic Form"}
...
```{r fig-sst-comparison-sorted-v2}
...
```{r fig-chisq-poisson-mixture-fixed}
...
\def\lenNoise{4} \def\lenSignal{6}
\coordinate (O) at (0,0);
...
\draw[thick] (P_SSE) -- ++(0, -0.2) node[below] {\textbf{$X_2$}};
\draw[thick] (P_SST) -- ++(0, -0.2) node[below] {\textbf{$X_2+X_1$}};

\draw[ultra thick, red!70!black] (O) -- (P_SSE);
\draw[ultra thick, blue!70!black] (P_SSE) -- (P_SST);

\draw[decorate, decoration={brace, mirror, amplitude=6pt, raise=5pt}, thick, red!70!black] 
    (O) -- (P_SSE) node[midway, below=12pt, align=center] {\textbf{$X_2$ (Noise)} \\ $\sim \chi^2(\text{df}_2)$};
\draw[decorate, decoration={brace, mirror, amplitude=6pt, raise=5pt}, thick, blue!70!black] 
    (P_SSE) -- (P_SST) node[midway, below=12pt, align=center] {\textbf{$X_1$ (Signal)} \\ $\sim \chi^2(\text{df}_1, \lambda)$};
\end{tikzpicture}
```
...
#| echo: false
#| fig-height: 3.5
#| fig-cap: "Densities of Non-Central F ($\\lambda$ defined as sum of squares)."

library(ggplot2)
x_f <- seq(0, 6, length.out = 300)
# Use l for R's ncp argument
df_list <- lapply(c(0, 2.5, 5), function(l) {
  data.frame(x = x_f, y = df(x_f, 5, 20, ncp = l), lambda = as.factor(l))
})
...
df_list <- lapply(c(0, 2.5, 5), function(l) {
  data.frame(x = x_b, y = dbeta(x_b, 2.5, 10, ncp = l), lambda = as.factor(l))
})
plot_data <- do.call(rbind, df_list)
...
for(l in c(0, 2.5, 5)) {
  # B_II = 1 - B_I. 
  y_val <- dbeta(1 - x_common, n1/2, n2/2, ncp = l)
  y_val[x_common > 1] <- 0
...
for(l in c(0, 2.5, 5)) {
  ...
  y_val <- (1/scale_factor) * dbeta(1 - val_mapped, n1/2, n2/2, ncp = l)
