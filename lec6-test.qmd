...
## Testing Reduced Model vs Full Model (Partial F-test)
...
%| fig-cap: "Geometric interpretation of the Full vs Reduced Model. The vector y is projected onto the Full Model space C(X) to get y_hat, and onto the Reduced Model space C(X1) to get y_hat0."
%| echo: false
%| fig-align: "center"
...
\begin{tikzpicture}[scale=1.2]
  % Define coordinates
  \coordinate (O) at (0,0);
  \coordinate (X1) at (4,0.5); % Reduced model space vector
  \coordinate (X2) at (1.5,3); % Another vector in Full model space
  \coordinate (Y) at (1.5,4.5); % Observation y

  % Draw plane C(X)
  \fill[blue!10] (0,0) -- (5,0.625) -- (6.5,3.625) -- (1.5,3) -- cycle;
  \node at (5.5,3) [blue] {$C(X)$ (Full Model)};

  % Draw line C(X1)
  \draw[thick, blue] (O) -- (5,0.625);
  \node at (4.5,0.3) [blue] {$C(X_1)$ (Reduced Model)};

  % Projections
  \coordinate (Yhat) at (2.5,2.2); % Projection onto C(X)
  \coordinate (Yhat0) at (2.0, 0.25); % Projection onto C(X1) approx

  % Draw vectors
  \draw[->, thick] (O) -- (Y) node[above left] {$y$};
  \draw[->, thick, red] (O) -- (Yhat) node[below right] {$\hat{y} = P_{C(X)}y$};
  \draw[->, thick, red] (O) -- (Yhat0) node[below] {$\hat{y}_0 = P_{C(X_1)}y$};

  % Difference vectors
  \draw[dashed] (Y) -- (Yhat);
  \draw[dashed] (Yhat) -- (Yhat0);
  \draw[->, thick, green!60!black] (Yhat0) -- (Yhat) node[midway, left] {$\hat{y} - \hat{y}_0$};
  \draw[dashed] (Y) -- (Yhat0);

  % Right angles
  \draw (Yhat) -- ++(0.2,0.1) -- ++(-0.1,0.2); % roughly perp to plane
...
::: {#thm-quadratic-independence}

### Distribution of Quadratic Forms
Suppose $y \sim N(X\beta, \sigma^2 I)$ where $X$ is $n \times (k+1)$ of full rank, $X\beta = X_1\beta_1 + X_2\beta_2$, and $X_2$ is $n \times h$. Let $\hat{y} = P_{C(X)}y$, $\hat{y}_0 = P_{C(X_1)}y$, and $\mu_0 = P_{C(X_1)}\mu$. Then:
...
### F-Test for Reduced Model
Under the conditions of @thm-quadratic-independence, the statistic
...
::: {#thm-overall-regression}

### Overall Regression F-Statistic
The test statistic for overall regression is given by:
...
If $y \sim N(X\beta, \sigma^2 I)$ and $C$ has rank $q$, then under $H_0: C\beta = 0$ :
...
### General Linear Hypothesis as Nested Models
The F-test for the general linear hypothesis $H_0: C\beta = 0$ is equivalent to a full-and-reduced model test. Specifically, testing $H_0$ is equivalent to testing whether the mean vector $\mu$ lies in a subspace $V_0 \subset C(X)$. The test statistic satisfies:
...
$$
\text{SSH} = ||(P_{\text{Col}(X)} - P_{V_0})y||^2 = ||P_{\text{Col}(Z)}y||^2
$$
...
```{tikz}
%| label: fig-nested-models-geometry
%| fig-cap: "Geometric Interpretation of the General Linear Hypothesis"
%| echo: false
%| fig-align: "center"
%| out-extra: 'style="width: 80% !important;"'
%| engine.opts:
%|   extra.preamble: "\\usepackage{amssymb} \\usepackage{amsmath} \\usepackage{xcolor}"

\begin{tikzpicture}[font=\sffamily, scale=1.2]
  % Define colors
  \definecolor{fillcolor}{RGB}{255, 204, 51} % Yellowish-orange (for Col(Z))
  \definecolor{linecolor}{RGB}{70, 130, 200} % Light Blue (for lines and text)
  \definecolor{textcolor}{RGB}{230, 80, 50}  % Red/Orange (for Col(Z) text)
  \definecolor{v0color}{RGB}{100, 150, 250}  % Blue (for V0)

  % 1. Outer Universe Circle (R^n)
  \draw[linecolor, thick] (0,0) circle (4.0cm);
  \node[linecolor] at (3.5, -3.5) {\Large $\mathbb{R}^n$};

  % 2. The Full Model Space Col(X) - Large circle
  % Fill with yellow to represent the difference space Col(Z)
  \fill[fillcolor] (0,0) circle (3.0cm);
  \draw[linecolor, thick] (0,0) circle (3.0cm);

  % 3. The Reduced Model Space V0 - Inner circle
  % Fill with blue
  \fill[v0color] (0,0) circle (1.2cm);
  \draw[linecolor, thick] (0,0) circle (1.2cm);

  % 4. Inner Label (H0) inside V0
  % Using white text for better contrast on the blue background
  \node[white, align=center] at (0, 0) {\large $H_0:$\\[0.2em] \Large $\mu \in V_0$};

  % 5. Label for Col(Z) INSIDE the yellow region
  \node[textcolor, align=center] at (0, 2.1) {\large $\text{Col}(Z) = V_0^\perp \cap \text{Col}(X)$};

  % 6. Label for V0 definition
  \node[linecolor] (v0_label) at (0, -2.0) {\large $V_0 = \text{Col}(Z)^\perp \cap \text{Col}(X)$};
  \draw[->, linecolor, thick] (v0_label.north) -- (0, -1.25);
  
  % 7. Label for Col(X)
  \node[linecolor] (cx_label) at (3.8, 0) {\Large $\text{Col}(X)$};
  \draw[->, linecolor, thick] (cx_label.west) -- (3.05, 0);

\end{tikzpicture}
```

### F-Test for Non-Zero General Linear Hypothesis

::: {#thm-general-linear-hypothesis-non-homogeneous}

### F-Test for Non-Zero  General Linear Hypothesis

Consider the general linear model $y = X\beta + \epsilon$ with $\epsilon \sim N(0, \sigma^2 I)$. To test the non-homogeneous hypothesis $H_0: C\beta = t$ where $t$ is a known vector and $C$ has rank $q$, the test statistic is:
...
::: {.proof}
**Distributional Proof (Alternative)**
...
**Step 1: Distribution of the Linear Combination**
Recall that $\hat{\beta} \sim N_{k+1}(\beta, \sigma^2(X^T X)^{-1})$.
Under the null hypothesis $H_0: C\beta = t$, we define the random vector $\theta = C\hat{\beta} - t$. Its expected value is:
$$E[\theta] = C E[\hat{\beta}] - t = C\beta - t = 0$$

Its variance-covariance matrix is:
$$\text{Var}(\theta) = C \text{Var}(\hat{\beta}) C^T = C [\sigma^2(X^T X)^{-1}] C^T = \sigma^2 [C(X^T X)^{-1} C^T]$$

Let $V = C(X^T X)^{-1} C^T$. Since $C$ has full row rank $q$, $V$ is positive definite. Thus:
$$\theta \sim N_q(0, \sigma^2 V)$$

**Step 2: Formation of the Chi-Squared Variable**
A standard result in multivariate statistics states that if $x \sim N_q(0, \Sigma)$, then the quadratic form $x^T \Sigma^{-1} x \sim \chi^2_q$. Applying this to $\theta$:
...
**Step 3: Independence and the F-Ratio**
From the properties of Least Squares, $\hat{\beta}$ is independent of the residuals (and thus independent of $\text{SSE}$). Consequently, the numerator $Q_H$ is independent of the denominator $\text{SSE}$.
We know that $\text{SSE} / \sigma^2 \sim \chi^2_{n-k-1}$.
...
:::: {#exm-chemical-reaction}

### Chemical Reaction Data

Consider an experiment designed to optimize the yield of a chemical reaction. The explanatory variables are:
...
$$
y_1 = \beta_0 + 0.5\beta_3 x_1 + 0.5\beta_3 x_2 + \beta_3 x_3 + \epsilon
$$
$$
y_1 = \beta_0 + \beta_3 (0.5 x_1 + 0.5 x_2 + x_3) + \epsilon
$$
...
## Speficic Tests for Linear Combinations of $\boldsymbol{\beta}$
...
:::: {#exm-inference-linercomb}
**Specific Tests and Intervals Implementation**
...
```{r}
#| echo: true
#| label: specific-tests-intervals
#| message: false
library(knitr)
library(ggplot2)
